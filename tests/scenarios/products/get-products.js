...
export function ShouldBeGetProductByIdReturnOneProduct() {
    postman[Request]({
        name: "login",
        method: "POST",
      ...
      postman[Request]({
        ...
        post() {
          pm.test('should be return status code 200 when making the get product', () => pm.response.to.have.status(200))
          pm.test('should be return in less than 1s when call the get product', () => pm.expect(pm.response.responseTime).to.be.below(1000))
        }
      });
}
